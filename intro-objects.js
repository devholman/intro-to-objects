...
var getFullNames = function(objectArray){
    ...
    for (var i=0; i < objectArray.length; i++){
        var allName = objectArray[i]
        newArray.push(allName.first + " " + allName.last)
    }
    return newArray
}
...
//identify the words in the array
//count them

var getCounts = function(inputString){

    var arrayOfWords = inputString.toLowerCase().split(' ') //must lowercase string before splitting it
    ...
    for(var i=0; i < arrayOfWords.length; i++){
        ...
        if (objectForWords.hasOwnProperty(word) === true) {

            objectForWords[word] += 1
        } else {

            objectForWords[word] = 1 
        }
    }
        return objectForWords
}
...
var generateDog = function(inputName){
    ...
    this.speak = function(inputString){
        var dogSpeak = inputString.split(' ')

        for(i=0; i < dogSpeak.length; i++){

            dogSpeak[i] = 'r' + dogSpeak[i].substr(1)
            
            console.log(dogSpeak)
           
        }
        return dogSpeak.join(' ')
    } 
}

//google .substring & .slice
...
var pluck = function(inputArray, prop){
    
    var namesArray = []

    for(i=0; i < inputArray.length; i++){
        
        var nameObject = inputArray[i]

        namesArray.push(nameObject[prop])
    }
...
log(flippedUsers[0]['president@gmail.com'] === 'obama')
log(flippedUsers[1]['americamoneywin@yahoo.com'] === 'trump')
log(flippedUsers[1].dealmaking === 'hobby')
